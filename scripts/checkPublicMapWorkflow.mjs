...
assert(selectedArea.includes("allowPrint &&") && selectedArea.includes('tractRecord.record?.screening.state === "potential_access_gap"') && selectedArea.includes("selectedTownCounts?.potential_access_gap") && selectedArea.includes("counts?.potential_access_gap"), "Print brief must appear only for a potential-gap tract or a county/town context containing at least one potential-gap tract.");
...
assert(autocomplete.includes("normalizedName.startsWith(query)") && autocomplete.includes("maximumSuggestions = 16") && autocomplete.includes('placeholder="Search a county or town..."'), "Boundary autocomplete must support one-character prefix suggestions with a bounded result list.");
