...
assert(storyPage.includes("An independent student-built public-data project") && storyPage.includes("Explore local care evidence across New Jersey.") && storyPage.includes("Four terms make the map easier to understand") && storyPage.includes("What is the doctor-office pilot?") && storyPage.includes("The next milestone is outside validation") && storyPage.includes("Independent project; not a government service") && storyPage.includes("Contact the project") && !storyPage.includes("github.com/methmoussa/careatlas-nj"), "The build story must explain the project and doctor-office pilot in plain language, retain its limitations and offer a contact path without publishing a source-code link.");
...
assert(header.includes('mapMode === "gaps"') && header.includes("How gaps are found") && header.includes("MethodologyGuide"), "The gap mode must expose the methodology guide without adding a separate route.");
...
assert(mapPage.includes('requestedMode === "gaps"') && mapPage.includes("hasSelectedArea ? \"gaps\" : \"healthcare\"") && mapPage.includes('url.searchParams.set("mode", "gaps")'), "The public map must start with facilities while legacy area links still restore gap mode.");
...
assert(addressFinder.includes("Enter a town for its summary") && addressFinder.includes("its Census tract") && map.includes("!pendingAddressMatch && (") && map.includes("<SelectedAreaCard") && map.includes("navigationOverrideActive"), "Place search must explain town and address input and show one settled tract result instead of flashing interim cards and map moves.");
