...
const officialSourceName = "HRSA Health Center Service Delivery and Look-Alike Sites";
const officialSourceHost = "data.hrsa.gov";
...
function printHelp() {
  console.log("Apply conservative, state-scoped review rules to staged official HRSA rows.");
  console.log("Usage: npm run review:hrsa-state -- --state=PA [--write]");
...
function sourceIsOfficial(record) {
  if (record?.sourceType !== "official" || record?.sourceName !== officialSourceName) return false;
...
function reviewReasons(record, state, stateBounds, duplicateCounts) {
  ...
  if (!sourceIsOfficial(record)) reasons.push("official HRSA source metadata is incomplete");
  ...
  if (facility?.isDemoData === true || facility?.verificationStatus === "demo" || demoPattern.test([
    facility?.id, facility?.name, facility?.sourceInfo?.notes, record?.sourceFile
...
async function main() {
  ...
  const stateRecords = records.filter((record) => normalizeStateCode(record?.state) === args.state);
  ...
  const nextRecords = records.map((record) => {
    if (normalizeStateCode(record?.state) !== args.state) return record;
    ...
    if (!reasons.length) {
      ...
      return {
        ...
        reviewNotes: "Official HRSA source metadata, identity fields, state, coordinates, and duplicate checks passed the conservative regional import review.",
        reviewedAt
      };
    }
  ...
  console.log(`Official HRSA staged review for ${args.state}`);
...
main().catch((error) => {
  console.error("Official HRSA staged review failed.");
